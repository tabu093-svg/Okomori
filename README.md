<link rel="stylesheet" href="assets/css/style.scss">

<div id="top"></div>


# 御籠りの行動予定

## 目次

- [12月27日(土)｜基地設営](#day-1227)
- [12月28日(日)｜境内準備・物品仕分け](#day-1228)
- [12月29日(月)｜28日の残作業](#day-1229)
- [12月30日(火)｜予備日](#day-1230)
- 
- [12月31日(水)9:00〜｜境内準備](#day-1231-1)
- [12月31日(水)15:00〜｜奥条区大祓い式](#day-1231-2)
- [12月31日(水)16:00〜｜厄才大祓い式](#day-1231-3)
- [12月31日(水)17:00〜｜火入れ〜御籠り本番](#day-1231-4)
- [1月1日(木)1:00〜｜片付け](#day-0101-1)
- [1月2日(金)｜奥条字厄才厄除祈願式・打ち上げ](#day-0102)
- [1月3日(土)｜残作業](#day-0103)
- 
- [納品など](#ext001)


<div style="page-break-after: always;"></div>

<h1 id="day-1227">📅 12月27日(土)｜基地設営</h1>

- 👬 **対象**：祐輔、紀夫、克昌、柿田、タカヤ  
- 🕘 **時間**：9:00〜12:00目処
- 🏫 **集合場所**：オクトピア

---

### 🏫オクトピア内の基地設営
- ブルーシートの設置
- 机、椅子の設置
- その他(冷蔵庫など)の設置
- 発注品の受領
    - マンセイ堂、キヨハチさん

---

### 🍵準備期間の休憩物資準備
- お茶500mlボトル ー24本x2箱
- ボトルコーヒー
- お菓子

---  

### 📝 メモ
- **20時頃〜**  
  保存会の練習休憩時間に  
  挨拶＋お菓子の差し入れ
- **法被借用(3Lx2、Lx10、Mx6)**
  1月中にクリーニングして返却

---

<div style="page-break-before: always;"></div>

<h1 id="day-1228">📅 12月28日(日)｜境内準備・物品仕分け</h1>

- 👬 **対象**：全員  
- 🕘 **時間**：9:00〜終日（終了まで）  
- 🏫 **集合場所**：オクトピア  
- 🚙 **車両**：2tトラック×2台／軽トラ×1台  

---  

### ⛩️ 境内準備
- 机・椅子の養生（オクトピア内）
- オクトピアからの運搬
- ハソリ、テントの運搬  
  - 旧市役所から借用
- テント組み立て
- ~~消防用ポンプ、ホース（6本）の運搬~~27日済み(7本)  
  - ~~魚楽近くの消防倉庫から借用~~

---

### 💡境内照明準備
- 森田電気さんと打ち合わせ
- 電球の運搬、設置

---

### 🗃️ 物品受領・仕分け
- マンセイ堂／キヨハチ／和さんからの物品受領
- 物品の仕分け

---

<div style="page-break-before: always;"></div>

### 🚗 買い出し
- 御籠りに必要な物資の買い出し
<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <thead>
    <tr>
      <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">物品</th>
      <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: right;">数</th>
    </tr>
  </thead>
  <tbody>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">おたま</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">4本</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">雑巾</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">30枚</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">厚手のウエットシート</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">5袋</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">ビニール袋（大）</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">10束</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">チャッカマン</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">2本</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">ガムテープ/養生テープ</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">各5個</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">金たわし / スポンジ</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">各3個</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">食器用洗剤</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">2本</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">紙コップ</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">1500個</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">袋入り割り箸</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">1500個</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">ゴム手袋（調理用）</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">30枚</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">つけてみそかけてみそ</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">3個</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">チューブからし</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">5本</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">一味唐辛子（瓶）</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">3本</td>
    </tr>
    <tr style="page-break-inside: avoid;">
      <td style="padding: 2px 6px; border: 1px solid #ccc;">使い捨てカイロ</td>
      <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">50個</td>
    </tr>
  </tbody>
</table>

---

### 📝 メモ
- **雨天の場合**  
  オクトピア屋内でできる作業をする  
  物品の仕分け、机イスの養生など  
- **昼食が出ます**
- **20時頃〜**  
  保存会の練習休憩時間に  
  挨拶＋お菓子の差し入れ

---

<div style="page-break-before: always;"></div>

<h1 id="day-1229">📅 12月29日(月)｜28日の残作業</h1>

👬　全員  
- 👬 **対象**：全員  
- 🕘 **時間**：9:00〜12;00目途  
- 🏫 **集合場所**：オクトピア  
- 🚙 **車両**：2tトラック×2台／軽トラ×1台 

### 📝 メモ
- **昼食が出ます**
- **20時頃〜**  
  保存会の練習休憩時間に  
  挨拶＋お菓子の差し入れ

---

<h1 id="day-1230">📅 12月30日(火)｜予備日</h1>

### 重機の搬入
- 境内に搬入
　- 運搬車両はオクトピアへ

### 📝 メモ
- **順調にいけば何も無し**

<div style="page-break-before: always;"></div>

---

[↑ ページ上部へ](#top)

<h1 id="day-1231-1">📅 12月31日(木)9:00〜｜境内準備</h1>
👬　全員  
🕛　9:00〜12:00目処  
🏫　オクトピア  
🚙　2tトラックx2台  軽トラx1台

- テント周辺の設営
    - テントの組み立て
    - 机/イスの設置
    - ごみ箱/食べ残し用バケツの設置
    - ストーブの準備
    - 発電機の準備
- 振る舞いの準備
    - おでん/甘酒/お神酒をオクトピアから運搬
    - ガスの受領
        - 両元さん
    - ハソリの設置
    - お菓子の運搬
- 照明の設営
    - 森田電気さんと設営
        - 完全に任せられる場合はお願いする
        - 配置した電球を確認して、  
          片付け時に見落とさないようにする
- 御神火周辺の準備
    - 防災の小屋からホースを引く
        - 山の中作業なので、けがには気を付けて！
    - 消防ポンプの設置
    - 御神火周辺に排水ミゾを側溝まで掘る
    - 火入れ式準備  
      松明に布を針金で巻いたものを  
      2本用意 
---
<div style="page-break-before: always;"></div>

<h1 id="day-1231-2">📅 12月31日(木)15:00〜｜奥条区大祓い式</h1>
👬　祐輔、紀夫    
**礼服にて参列**



---

<h1 id="day-1231-3">📅 12月31日(木)16:00〜｜厄才大祓い式</h1>
👬　全員    
**服装自由**

---

<div style="page-break-before: always;"></div>

<h1 id="day-1231-4">
    📅
    <font color="Red"> 
         12月31日(木)　17:00〜　火入れ式〜御籠り本番 
    </font>
</h1>

- 🔴フリー遊撃班   -**祐輔、紀夫、克昌**
  - 基本的にフリー
  - 人手不足のヘルプ
  - ０時過ぎ頃、新年の挨拶参加

***  

- 🔥御神火班     -**田中、室井** 
   - 水撒きをしながら火力調整
   - 0時にお囃子が到着するので  
     松の葉を投入して火力MAX!!!!

***  

- 🍶お神酒班    -**ヒロヤス、いくや**
    - お神酒を参拝客に振る舞う

***  

- 🍢振る舞い班    -**柿田、中井、まっとー**
    - おでんを参拝客に振る舞う
    - 甘酒を参拝客に振る舞う

***  

- 🥳接待班    -**カズキ** 
    - テントの案内全般
    - 薬味等の案内
    - 残飯、ゴミ箱の案内

***  

- 🍬お菓子班    -**タカヤ、シュンスケ(24厄)**
    - 参拝の帰りに、お菓子を配る

---

[↑ ページ上部へ](#top)


<div style="page-break-before: always;"></div>

<h1 id="day-0101-1">
    📅
    <font color="Red"> 
         1月1日(金)　1:00〜5:00目処 片付け
    </font>
</h1>

---

### 🔥御神火の片付け
- 御神火の消化
- 御神火の解体
    - 燃え残りを**重機**で解体して、2tトラックに積む
        - 積んだ後、仮置き場まで運搬
    - 解体時に、鎹を回収する(28本)
    - 消防ポンプ、ホースの片づけ
- **運搬物がすべてオクトピアへ運ばれたら**  
  **御神火跡に砂利を戻して整地する**

---

### ⛺️振る舞い周辺の片付け
 
- 物品の片づけ(境内)
    - テント/椅子
        - 市役所から借用したものは番号が  
          振ってあるので番号通りに片付け
    - ハソリ
    - 飾り付け
    - ゴミ/残飯
- 物品の運搬
    - 御神火解体が完了し、導線が確保できたら
      オクトピアへ運搬
    - 太太神楽式用の分は倉庫へ運搬  
      **机(脚長)12台／机(脚短)2台／椅子15脚**
- 物品の片づけ(オクトピア)
    - ハソリを洗う
    - 机／椅子の片づけ
    - ごみの処理

---

### 💡照明の片付け
- 照明を回収、収納箱に入れる
    - **暗く、高い所にも設置されているので気を付けて作業**

---

<div style="page-break-before: always;"></div>

### 📝 メモ
- 目途がついたら1～２時間仮眠
- ゴミ回収センターが8：00～開くので御神火ゴミを運搬
- **10時～元旦祭があるので境内をきれい最終清掃**
- すべての作業が完了したら翌日に向けてゆっくり休んでください！

[↑ ページ上部へ](#top)

---

<div style="page-break-before: always;"></div>

<h1 id="day-0102">📅 1月2日(金)|奥条字厄才厄除祈願式・打ち上げ</h1>

---
### 奥条字厄才厄除祈願式
- 👬 **対象**：全員  
- 🕘 **時間**：8:30〜午前中
- **礼服で参列してください**

---
### 打ち上げ
- 👬 **対象**：全員  
- 🕘 **時間**：14：00～
- 全力で楽しんでください！！！！  
  必要な方は千円札をたくさん用意してください！！！

<div style="page-break-before: always;"></div>

<h1 id="day-0103">📅 1月3日(土)|残作業</h1>

---

### 残作業
- 消防ポンプ、ホースの返却
    - 消防倉庫
- トラックの返却
    - 市田建設
    - レンタカー屋
- ハソリ／テントの返却
    - 市役所
- ストーブ(空焚きして返却)
    - 奥条区
- 発電機の返却(ガソリン満タン)
- 白布等の返却(クリーニングする)
    - 奥条区
- 神社境内の蔵のカギの返却
    - 氏子総代

[↑ ページ上部へ](#top)

<div style="page-break-before: always;"></div>

<h1 id="ext001">📅 12月27～29日 納付など</h1>

### 喜代八さん
#### 御籠り用

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: center;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: right;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">適用</th>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">木升</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">400個</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">100個は祭礼用</td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">ひしゃく</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">2本</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">3</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">御神酒</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">4斗樽（36L）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">1樽</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">熱燗用含む</td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">4</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">塩</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">2袋</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">5</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">飾り樽</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">一式</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">6</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">お茶</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">2L</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">36本</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">7</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">水</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">2L</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">18本</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>
</table>

#### ~~保存会差し入れ用~~ 27日受領

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: center;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: right;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">適用</th>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">ビール</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">24本（1ケース）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">お茶（ペットボトル）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">24本（1ケース）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>
</table>

### マンセイ堂
#### お籠り振る舞い用(12/27納品予定)

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">適用</th>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">かっぱえびせん（小）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">24袋入り</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">21箱</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">サッポロポテト（ベジタブル）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">24袋入り</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">21箱</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc;">3</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">サッポロポテト（バーベQ）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">24袋入り</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">21箱</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc;">4</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">ポテトチップス</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">12袋入り</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">3箱</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">少年少女囃子 お土産用</td>
  </tr>
</table>

#### 厄才厄除祈願式用(1/2 8:00納品予定)

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: center;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: right;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">適用</th>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">二升餅 かざり餅</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">1式</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">二合餅（紅白）</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">50組</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">3</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">お菓子</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">50個</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>

  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">4</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">紙袋</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">70枚</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">12/29引取り</td>
  </tr>
</table>


#### 大祓式用(12/31納品予定)

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: center;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: right;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">適用</th>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">二升餅　飾り餅></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">2本</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
  </tr>
</table>


### 両元産業12/29AM

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc;">適用</th>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">LPガスボンベ</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">20kg</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">3本</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">おでん、湯沸し用</td>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">LPガスボンベ</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">20kg</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">1本</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">予備</td>
  </tr>
</table>

### 日赤(常滑市)借用品

<table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
  <tr>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: center;">No.</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">品名</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">規格</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: right;">数量</th>
    <th style="padding: 3px 6px; border: 1px solid #ccc; text-align: left;">適用</th>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">1</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">はそり</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">3</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">No.8,9,14</td>
  </tr>
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">テント</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;"></td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">2</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">No.2,7 </td>
  </tr>
  <!-- 空欄行① -->
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">3</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">　</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">　</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">　</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">　</td>
  </tr>
  <!-- 空欄行② -->
  <tr style="page-break-inside: avoid;">
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: center;">4</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">　</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">　</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc; text-align: right;">　</td>
    <td style="padding: 2px 6px; border: 1px solid #ccc;">　</td>
  </tr>
</table>


[↑ ページ上部へ](#top)
